...
create or replace function
validate_email(email text, token uuid)
  returns void
  language plpgsql
  as $$
declare
...
begin
  if exists(select 1 from basic_auth.tokens
             where tokens.email = validate_email.email
               and tokens.token = validate_email.token
               and token_type = 'validation') then
    raise notice 'Validatin that email';
    update basic_auth.users set verified=true
     where users.email = validate_email.email;
...
ALTER DATABASE test SET postgrest.claims.email TO '';
...
-- XXX: the following is too permissive!!
grant select, insert, update, delete
  on basic_auth.tokens, basic_auth.users to anon;

grant usage on schema public, basic_auth to anon;

-- anon can create new logins
grant insert on table basic_auth.users, basic_auth.tokens to anon;
grant select on table pg_authid, basic_auth.users, basic_auth.tokens to anon;
grant execute on function
  login(text,text),
  request_password_reset(text),
  reset_password(text,uuid,text),
  signup(text, text),
  validate_email(text,uuid)
  to anon;
